...
export function AddLessonDialog({ open, onOpenChange, onAdd, defaultDate, isSubmitting }: AddLessonDialogProps) {
  ...
  const isoDate = format(form.date, "yyyy-MM-dd");

  // Fetch existing lessons for the selected date to detect conflicts
  const { data: existingLessons = [] } = useQuery({
    queryKey: ["conflict-check", isoDate],
    queryFn: async () => {
  ...
  const conflict = useMemo(() => {
    const newStart = form.startTime.replace(":", "");
    const newEnd = form.endTime.replace(":", "");
    for (const l of existingLessons) {
      const lStart = (l.start_time?.slice(0, 5) ?? "").replace(":", "");
      const lEnd = (l.end_time?.slice(0, 5) ?? "").replace(":", "");
      if (newStart < lEnd && newEnd > lStart) {
        return l.student_name;
      }
    }
    return null;
  }, [form.startTime, form.endTime, existingLessons]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.studentId || isSubmitting) return;
    onAdd({
    ...
    <form onSubmit={handleSubmit} className="space-y-4 p-1">
      ...
      {conflict && (
        <div className="flex items-center gap-2 p-3 rounded-lg bg-destructive/10 border border-destructive/30 text-destructive text-sm">
          ...
          <span>Conflito de horário com <strong>{conflict}</strong></span>
        </div>
      )}
      ...
      {isMobile ? (
        <Drawer open={open} onOpenChange={handleOpenChange}>
          <DrawerContent className="max-h-[90vh]">
            <DrawerHeader>
              <DrawerTitle>Nova Aula</DrawerTitle>
            </DrawerHeader>
