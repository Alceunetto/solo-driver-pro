...
export default function Financeiro() {
  ...
  const horasTrabalhadas = transactions.filter((t) => t.type === "entrada").length; // 1h per lesson simplified
    ...
    <div className="container py-6 pb-24 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="glass-card p-4">
          ...
          <p className="text-xl font-bold text-success">R$ {totalEntradas.toLocaleString()}</p>
        </div>
        <div className="glass-card p-4">
          <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-xl font-bold text-destructive">R$ {totalSaidas.toLocaleString()}</p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-muted-foreground font-medium">ROI por Hora</span>
              </div>
              <p className={`text-3xl font-bold ${roiPorHora >= 0 ? "text-success" : "text-destructive"}`}>
            ...
            <div className="text-right">
              <div className="flex items-center gap-2 mb-1 justify-end">
                ...
                <span className="text-xs text-muted-foreground font-medium">Lucro Líquido</span>
              </div>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button className="h-12 text-sm font-semibold">
              <PlusCircle className="w-4 h-4 mr-1" /> Lançamento
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-4 pt-2">
              ...
              <Button onClick={addTransaction} className="w-full">Salvar</Button>
            </div>
          </DialogContent>
        </Dialog>

        <Button onClick={generatePixLink} variant="outline" className="h-12 text-sm font-semibold">
          <QrCode className="w-4 h-4 mr-1" /> Cobrar via Pix
        </Button>
      </div>
          ...
          {transactions.map((tx) => (
            <div key={tx.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
              <div className="flex items-center gap-3">
                <div className={`p-1.5 rounded-md ${tx.type === "entrada" ? "bg-success/10" : "bg-destructive/10"}`}>
                  {tx.type === "entrada" ? (
